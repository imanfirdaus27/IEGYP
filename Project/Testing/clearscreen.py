...
def cardetails(car_id, rental_days):
    global car_data
    if car_id in car_data:
        details = car_data[car_id]
        ...
        print(f"{'Grand Total':<20}:{' ':70} RM{total_rental_payment:.2f}")
...
def credit_card(customer_id):
    while True:
        credit_card_no = keyboardInput(str, "Enter the credit card number: ", "Credit Card Number must be an integer").replace(" ", "").strip()
        ...
        try:
            while True:
                expired_date = keyboardInput(str, "Enter credit card expired date in format MMYY without '/': ", "Expired date must be an integer").strip()
                ...
                while True:
                    cvv = keyboardInput(str, "Enter CVV number: ", "CVV must be an integer").strip()
...
def calculate_payment(customer_id):
    ...
    if customer_id:
        ...
        if balance < total_payment:
            ...
            cust_data[customer_id]['Balance'] = f"RM {new_balance:.2f}"
            print("Your payment is successful")
...
def cash_payment(customer_id):
    username_password(customer_id)
    update_balance(customer_id, "Cash Payment (PAID)")

def update_balance(customer_id, payment_method):
    global cust_data
    if customer_id in cust_data:
        cust_data[customer_id]['Payment Method'] = payment_method
        print("Payment is updated to", payment_method)
    else:
        print("Invalid Customer ID")
...
def save_data():
    global cust_data, car_data
    try:
        with open('customerdetails.txt', "wt") as cust_detail:
            cust_detail.write(str(cust_data))

        with open('carlisting.txt', "wt") as car_listing:
            car_listing.write(str(car_data))

    except Exception as e:
        print("Something went wrong while printing the data:", e)
...
def main():
    ...
    start_date, end_date = customerdetails(customer_id)
    car_id = getcarID(customer_id)

    if start_date and end_date and car_id:
        rental_days = calculate_rental_days(start_date, end_date)
        total_rental_payment = cardetails(car_id, rental_days)

        payment_method(customer_id)

        # Clear screen and show final invoice after delay
        clear_screen()
        time.sleep(3)  # Delay for 3 seconds
        print_invoice(customer_id, total_rental_payment)

    save_data()

def print_invoice(customer_id, total_rental_payment):
    global cust_data, car_data
