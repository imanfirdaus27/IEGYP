...
def credit_card(customer_id):
    while True:
        credit_card_no = keyboardInput(str, "Enter the credit card number (16-Digits): ", "Credit Card Number must be an integer").replace(" ", "").strip()
        ...
        try:
            while True:
                ...
                while True:
                    cvv = keyboardInput(str, "Enter CVV number(3-Digits): ", "CVV must be an integer").strip()
        ...
        print_invoice(customer_id, payment_method)
...
#     new_balance = calculate_payment(customer_id)
#     if new_balance is not None:
#         cust_data[customer_id]['Balance'] = f"RM {new_balance:.2f}"
#         with open('updated_customerdetails.txt', 'w') as file:
#             file.write(str(cust_data))
...
def print_invoice(customer_id, payment_method):
    global cust_data, car_data
